...
### AI·ML 시스템 선정 보강 (`design-principles-ai-system-quality`)

- **준법·규제 NFR**: 고영향 AI 등 법적 설명·공정성·기록 의무는 **성능 NFR과 교환하지 않고** NFR로 고정 가능.
- **안전**: 생명·안전 직결 시 강건성·제어 가능성은 다른 QA와의 **스왑 대상이 아님**을 `qualities.md`에 명시.
- **응용 유형**: `docs/ai_sw_design_methodology.md` §5 표를 참고해 우선순위를 비즈니스와 합의하고, 충돌 시 **완화 전략**(XAI, PETs, 압축, 임계 재조정)을 근거와 함께 남긴다.
- **Trade-off**: 품질 강화 vs 수행 효율성, 공정성·프라이버시·설명성 vs 기능 정확성 등 상충을 표로 한 번 요약한다.
